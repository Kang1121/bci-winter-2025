...
# from transformers_local import ViTForImageClassification as AutoModelForImageClassification
...
class ImageClassifierTrainer:
    def __init__(self, DATA, model_path, sub = '', num_labels=5, lr=5e-5, batch_size=128):
        ...
        self.model.classifier = Linear(self.model.config.hidden_size, self.num_labels)
    ...
    def train(self, epochs=3, lr=None, freeze=True, log = False):
        ...
        for epoch in range(epochs):
            ...
            time_end = time.time()
            print(f"Epoch {epoch + 1}, Test Accuracy: {avg_accuracy * 100:.2f}%, Time: {time_end - time_start:.2f}s")
    ...
    def save_model(self, sub):
        ...
        model_path = "ckpt/vision_new"
        os.makedirs(model_path, exist_ok=True)
        model_save_path = f'{model_path}/model_vision_classification_sub{sub:02d}.pth'
...
if __name__ == '__main__':
    ...
    for idx in range(34, 43):
        ...
        trainer = ImageClassifierTrainer(data,
                                         ...
                                         num_labels=5, lr=5e-5, batch_size=64)
