...
class ASTSdpaSelfAttention(ASTSelfAttention):
    def __init__(self, config: ASTConfig, embed_dim, num_heads) -> None:
...
class ASTSelfOutput(nn.Module):
    ...
    def __init__(self, config: ASTConfig, embed_dim=None, num_heads=None) -> None:
        super().__init__()
        if not embed_dim:
...
class ASTAttention(nn.Module):
    def __init__(self, config: ASTConfig, embed_dim=None, num_heads=None) -> None:
        ...
        self.output = ASTSelfOutput(config, embed_dim, num_heads)
...
class ASTSdpaAttention(ASTAttention):
    def __init__(self, config: ASTConfig, embed_dim=None, num_heads=None) -> None:
        ...
        self.attention = ASTSdpaSelfAttention(config, embed_dim, num_heads)
...
class ASTMLPHead(nn.Module):
    def __init__(self, config: ASTConfig):
        ...
        self.layernorm = LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dense = Linear(config.hidden_size, config.num_labels)
...
class ASTForAudioClassification(ASTPreTrainedModel):
    ...
    def relprop(self, cam=None, method="transformer_attribution", is_ablation=False, start_layer=0, **kwargs):
        return self.audio_spectrogram_transformer.relprop(cam, method, is_ablation, start_layer, **kwargs)
    ...
    def relprop_classifier(self, cam=None, method="transformer_attribution", is_ablation=False, start_layer=0, **kwargs):
        ...
        cam = self.audio_spectrogram_transformer.relprop(cam, method, is_ablation, start_layer, **kwargs)

        if method == "transformer_attribution" or method == "grad":
            cams = []
            for layer in self.audio_spectrogram_transformer.encoder.layer:
